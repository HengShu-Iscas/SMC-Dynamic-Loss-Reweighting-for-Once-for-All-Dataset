...
def hutchinson_trace_input(loss, inputs, n_samples=1):
    """
    Hutchinson estimator for trace(H) where H = d^2 loss / d inputs^2.
    - loss: scalar tensor
    - inputs: tensor with requires_grad=True (e.g., the synthesized batch img_syn)
    - n_samples: number of Hutchinson probe vectors (1..3 suggested)
    Returns scalar tensor approximating trace(H).
    Notes:
    - If inputs.requires_grad is False, returns 0.0 on the same device.
    - Uses Rademacher probes (+1/-1).
    """
    if not getattr(inputs, "requires_grad", False):
        # If inputs don't require grad, curvature w.r.t inputs is irrelevant here.
        return torch.tensor(0.0, device=loss.device, dtype=loss.dtype)

    trace_est = 0.0
    for _ in range(max(1, int(n_samples))):
        # Rademacher probe +1/-1
        v = (torch.randint(0, 2, inputs.shape, device=inputs.device).to(inputs.dtype) * 2.0 - 1.0)
        # first-order gradient: d loss / d inputs
        grad1 = autograd.grad(loss, inputs, create_graph=True, retain_graph=True)[0]
        # directional second derivative: (grad1 * v).sum() -> gradient w.r.t inputs gives H v
        Hv = autograd.grad((grad1 * v).sum(), inputs, retain_graph=True)[0]
        trace_est = trace_est + (v * Hv).sum()
    trace_est = trace_est / float(max(1, int(n_samples)))
    return trace_est
...
def condense(args, logger, device='cuda'):
    ...
    for init_loop in range(resume_epoch, n_iter):
        ...
        for model_loop in range(args.inner_loop):
            ...
            for c in range(nclass):
                ...
                if  condition1 and condition2:
                    ...
                    loss_list = feat_loss_for_ipc_reg(args, img_aug[:n], img_aug[n:], model, indices=ipcx_index_class_dict, init_loop=init_loop, n_iter=n_iter, num_sampled=img.shape[0])
                    loss_list_per_class[c].append(loss_list)

                if use_reg_flag:
                    reg_ipcx_list = regularizer_list[c].get_regularized_ipc()
                    loss = grad_loss_for_img_update(args, img_aug[:n], img_aug[n:], lab, lab_syn, model, ipcx_list=reg_ipcx_list, indices=ipcx_index_class_dict, num_sampled=img.shape[0], init_loop=init_loop, n_iter=n_iter)
                else:
                    if args.match == 'feat' and args.use_mmd and args.metric == 'mse':
                        num_samples = min(len(img_aug[:n]), len(img_aug[n:]))
                        img_real_gpu = img_aug[:n]  
                        img_syn_gpu = img_aug[n:]  
                        ...
                        # img_real_aug = aug(sampled_real_imgs) 
                        # img_syn_aug = aug(sampled_syn_imgs)
                        ...
                        for i in range(len(feats_syn)):
                            loss_mse = add_loss(loss_mse, dist(feats_syn[i].mean(0), alpha * feats_holistic[i].mean(0) + (1 - alpha) * feats_real[i].mean(0), method='mse'))
                        for fr, fs, fh in zip(feats_real, feats_syn, feats_holistic):
                            fr_flat = fr.reshape(fr.size(0), -1)
                            fs_flat = fs.reshape(fs.size(0), -1)
                            fh_flat = fh.reshape(fh.size(0), -1)
                        ...
                        if args.gamma_schedule:
                            ...
                            γ = schedule_gamma(curr_iter=init_loop,
                                            max_iter=n_iter - 1,
                ...
                # --- Hessian-regularized update for synthetic images ---
                # hyperparams (can be added into args; fallback defaults used if not present)
                lambda_hess = getattr(args, 'lambda_hess', 0.0)      # strength of Hessian regularizer (0 to disable)
                hess_nsample = getattr(args, 'hess_nsample', 1)      # Hutchinson probe count (1..3)
                hess_every = getattr(args, 'hess_every', 1)          # apply every `hess_every` outer iter (1 => every iter)

                # compute optional Hessian regularizer only if enabled
                use_hess = (lambda_hess > 0.0) and ((init_loop % max(1, hess_every)) == 0)

                if use_hess:
                    # img_syn should be the synthesized batch tensor used to compute `loss`.
                    # In your loop img_syn is obtained earlier as:
                    #    img_syn, lab_syn, sampled_indices = synset.sample(...)
                    # and later used in augmentation as img_aug = aug(torch.cat([img, img_syn]))
                    # Here we compute trace(H) w.r.t. img_syn.
                    try:
                        h_est = hutchinson_trace_input(loss, img_syn, n_samples=hess_nsample)
                        # you can also clamp or scale the Hessian estimate to avoid instabilities
                        # e.g., h_est = torch.clamp(h_est, -1e6, 1e6)
                        loss_total_for_img = loss + lambda_hess * h_est
                    except RuntimeError as e:
                        # fallback: if second-order computation fails, skip Hessian for this step
                        # (avoid crashing the whole training)
                        logger(f"Warning: Hessian estimation failed at iter {init_loop}: {e}")
                        loss_total_for_img = loss
                else:
                    loss_total_for_img = loss

                optim_img.zero_grad()
                loss_total_for_img.backward()
                optim_img.step()
                ts.stamp("backward")
                # --- end Hessian-regularized update ---
        ...
        if condition1 or condition2:
            ...
            if use_reg_flag and (init_loop % args.adaptive_period == 0):
                logger(f"Saving regularizer list at iteration {init_loop + 1}")
                if regularizer_list:  
                    pickle.dump(regularizer_list, open(os.path.join(args.save_dir, f"regularizer_{init_loop+1}.pkl"), "wb"))
                else:
                    logger("Regularizer list is empty or uninitialized!")
        ...
        # if init_loop > 0 and (init_loop % args.print_freq == 0):
        #     logger(f"[Outer it {init_loop}/{n_iter}] γ={γ:.3f}, "
