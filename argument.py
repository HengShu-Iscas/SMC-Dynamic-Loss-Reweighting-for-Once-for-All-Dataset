...
parser.add_argument('--use_mmd',action='store_true',
                    help='Whether to complement MSE with MMD in feature matching.''if None,counting by ipc')
parser.add_argument('--gamma', type=float, default=None,
                    help='Balance weight γ FOR MSE vs MMD (0≤γ≤1).')
parser.add_argument('--mmd_kernel_mul', type=float, default=2.0,
                    help='Multiplier for Gaussian kernel bandwidth in compute_mmd')
parser.add_argument('--mmd_kernel_num', type=int, default=5,
                    help='Number of kernels (different banwidths) in compute_mmd')
parser.add_argument('--mmd_fix_sigma', type=float, default=None,
                    help='If set,use this fixed sigma instead of data-dependent sigma in compute_mmd')
parser.add_argument('--min_mmd', type=float, default=0.1,
                    help='下限：MMD 最低权重 (1–γ ≥ min_mmd)')
parser.add_argument('--min_mse', type=float, default=0.1,
                    help='下限：MSE 最低权重 (γ ≥ min_mse)')
parser.add_argument('--gamma_start', type=float, default=0.1,
                    help='If scheduling γ, 起始值（训练第 1 次迭代）')
parser.add_argument('--gamma_end',   type=float, default=0.8,
                    help='If scheduling γ, 结束值（训练最后一次迭代）')
parser.add_argument('--gamma_schedule', action='store_true',
                    help='是否开启 γ 随迭代线性增长调度')
...
parser.add_argument('--lambda_hess', type=float, default=0.0, help='Hessian trace regularizer strength for synth images')
parser.add_argument('--hess_nsample', type=int, default=1, help='Hutchinson probe count for Hessian estimation')
parser.add_argument('--hess_every', type=int, default=1, help='Apply Hessian regularizer every this many outer iters')
...
parser.add_argument('--adaptive_k', type=int, default=2,
                    help='在 select_reg_ipc 时，返回 top-K 个子集 (默认为 2)')
...
if args.ipc > 0:
    if args.slct_type == 'random':
        ...
        if args.use_mmd:
            args.run_name += '_mmd'
            if args.gamma is not None:
                args.run_name += f'_g{args.gamma:.1f}'
